...
<div class="max-w-3xl space-y-6">
    ...
    {{-- Header --}}
    <div>
        <a href="{{ route('admin.kabinet.index') }}" class="inline-flex items-center gap-2 text-sm text-slate-400 hover:text-white transition-colors mb-4">
            <i class="bi bi-arrow-left"></i> Kembali
        </a>
        <h1 class="text-2xl font-bold text-white">Edit Kabinet</h1>
        <p class="text-sm text-slate-400 mt-1">Perbarui data logo dan nama kabinet</p>
    </div>

    {{-- Form --}}
    <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 md:p-8">
        <form action="{{ route('admin.kabinet.update', $kabinet->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')

            <div>
                <label for="nama" class="block text-sm font-medium text-slate-300 mb-2">Nama Kabinet</label>
                <input type="text" name="nama" id="nama" value="{{ old('nama', $kabinet->nama) }}" required
                    ...
                    placeholder="Contoh: Kabinet Renascens">
                @error('nama')
            ...
            <div>
                <label class="block text-sm font-medium text-slate-300 mb-2">Logo Kabinet Saat Ini</label>
                @if($kabinet->logo)
                    <div class="mb-4">
                        <img src="{{ asset('storage/' . $kabinet->logo) }}" alt="Logo {{ $kabinet->nama }}" class="h-32 object-contain bg-white/10 rounded-lg p-2 border border-slate-700">
                    </div>
                @else
